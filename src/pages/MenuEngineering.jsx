        ...
        {!targetItem ? (
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Price slider — bounded to ±25% of current price.
                  The constant-elasticity demand model is only trustworthy
                  for small perturbations around the historical price;
                  beyond ±25% the projected qty/profit numbers are
                  extrapolation, not prediction. Hard-clamping the slider
                  is cleaner UX than letting it roam and then showing a
                  warning. */}
              <div className="p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                ...
                <input
                  type="range"
                  min={-Math.floor(targetItem.price * 0.25)}
                  max={Math.ceil(targetItem.price * 0.25)}
                  step={1}
                  value={priceDelta}
                ...
                <div className="flex items-center justify-between mt-1 text-[10px] text-gray-400">
                  <span>Min: SAR {Math.round(targetItem.price * 0.75)}</span>
                  <span>Max: SAR {Math.round(targetItem.price * 1.25)}</span>
                </div>
              </div>
